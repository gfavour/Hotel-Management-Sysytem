			...
			<!--box-header -->
				...
				<div class="box-body table-responsive no-padding" style="margin-top:10px;">
                  <form name="reservation" id="reservation">
					...
					$sql = select2nav("SELECT count(orders.id) as ctn FROM orders INNER JOIN addclient ON orders.guestid = addclient.id WHERE (orders.isbar2 = '1' OR orders.isswimpool = '1') AND (addclient.lastname LIKE '%".$_POST["q"]."%') ORDER BY orders.id DESC","SELECT orders.*,addclient.lastname,addclient.firstname FROM orders INNER JOIN addclient ON orders.guestid = addclient.id WHERE (orders.isbar2 = '1' OR orders.isswimpool = '1') AND (addclient.lastname LIKE '%".$_POST["q"]."%') ORDER BY orders.id DESC");
					
					}else{
					$sql = select2nav("SELECT count(orders.id) as ctn FROM orders INNER JOIN addclient ON orders.guestid = addclient.id WHERE (orders.isbar2 = '1' OR orders.isswimpool = '1') ORDER BY orders.id DESC","SELECT orders.*,addclient.lastname,addclient.firstname FROM orders INNER JOIN addclient ON orders.guestid = addclient.id WHERE (orders.isbar2 = '1' OR orders.isswimpool = '1') ORDER BY orders.id DESC");
					}
