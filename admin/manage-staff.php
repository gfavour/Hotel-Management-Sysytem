...
<div class="row">
            <div class="col-xs-12">
			  ...
			  <div style="margin:0 0 10px 0">
               <button class="btn btn-primary btn-sm" style="margin-right:10px;" id="jqbtn1"><i class="fa fa-plus"></i> Add New Staff</button></div>
				...
				<div class="box-body table-responsive no-padding" style="margin-top:10px;">
				  ...
				  echo '<table class="table table-striped">
					...
					while($rs = mysqli_fetch_assoc($qry)){
						$delBtn = ($rs["users_role"] == 'admin')?'<a href="javascript:;" class="label label-default">Delete</a>':'<a href="javascript:;" class="label label-danger" onclick="confirmdel(\'id='.$rs["id"].'&tbl=users&dwat=del\',\'../fxn/process1.php\',\'\')">Delete</a>';
						if($rs["email"] == ''){$email = ''; }else{ $email = $rs["email"]; }
						if($rs["email"] == ''){$email = ''; }else{ $email = $rs["email"]; }
						...
						echo '<td>'.$email.'</td>';
						echo '<td>'.strtoupper($rs["users_role"]).'</td>';
