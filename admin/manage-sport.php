...
<div class="row">
                ...
                <div class="box-header with-border">
				  ...
				  <div class="box-tools">
					 ...
					 <div class="input-group-btn">
						...
						<button class="btn btn-primary btn-sm" style="margin-right:10px;" id="jqbtn1"><i class="fa fa-plus"></i> Add Sport Item</button>
						<?php } ?>
		...
		<div id="msgbox"></div>
		<?php include("insert-new-sport.php"); ?>
			...
			<!--box-header -->
				...
				<div class="box-body table-responsive no-padding" style="margin-top:10px;">
                  <form name="reservation" id="reservation">
					<?php
					if ($_GET["dwat"] == 'search'){
					$sql = select2nav("SELECT count(id) as ctn FROM addsport WHERE (name LIKE '%".mysqli_real_escape_string($conn4as,$_GET["q"])."%') ORDER BY id DESC","SELECT * FROM addsport WHERE (name LIKE '%".mysqli_real_escape_string($conn4as,$_GET["q"])."%') ORDER BY id DESC");
					}else{
					$sql = select2nav("SELECT count(id) as ctn FROM addsport ORDER BY id","SELECT * FROM addsport ORDER BY id");
					}
					if($sql){
				  echo '<table class="table table-striped">
					...
					if ($globalrole == 'admin'){
						$delBtn = '<a href="javascript:;" class="label label-danger" onclick="confirmdel(\'id='.$rs["id"].'&tbl=addsport&dwat=del\',\'../fxn/process1.php\',\'\')">Delete</a>'; $editBtn = '<a href="editsport.php?id='.$rs["id"].'" class="label label-primary">Edit</a>';
					
					}elseif ($globalrole == 'manager'){
						$delBtn = '<a href="javascript:;" class="label label-default">Delete</a>'; $editBtn = '<a href="editsport.php?id='.$rs["id"].'" class="label label-primary">Edit</a>';
					}else{
