...
<div class="content-wrapper">
        ...
        <section class="content-header">
          <h1>Bar 2 (Pool Bar)</h1>
          <ol class="breadcrumb">
            <li><a href="#"><i class="fa fa-dashboard"></i> Home</a></li>
            <li class="active">Bar 2</li>
          </ol>
        </section>
...
<div class="row">
            <div class="col-xs-12">
			 ...
			 <div style="margin-bottom:10px">
					    <a href="order-bar2-home.php" class="btn btn-warning btn-sm" style="margin-right:10px;"><i class="fa fa-plus"></i> New Order</a>
						</div>
			  <?php } ?>
			...
			<!--box-header -->
				...
				<div class="box-body table-responsive no-padding" style="margin-top:10px;">
                  <form name="reservation" id="reservation">
					<?php
					if ($_GET["dwat"] == 'search'){
					$sql = select2nav("SELECT count(id) as ctn FROM addbar2 WHERE (name LIKE '%".mysqli_real_escape_string($conn4as,$_GET["q"])."%') ORDER BY id DESC","SELECT * FROM addbar2 WHERE (name LIKE '%".mysqli_real_escape_string($conn4as,$_GET["q"])."%') ORDER BY id DESC");
					}else{
					$sql = select2nav("SELECT count(id) as ctn FROM addbar2 ORDER BY id","SELECT * FROM addbar2 ORDER BY id DESC");
					}
					if($sql){
				  echo '<table class="table table-striped">
					...
					if ($globalrole == 'admin'){
						$delBtn = '<a href="javascript:;" class="label label-danger" onclick="confirmdel(\'id='.$rs["id"].'&tbl=addbar2&dwat=del\',\'../fxn/process1.php\',\'\')">Delete</a>';
						$restockBtn = '<a href="bar2-restock.php?id='.$rs["id"].'" class="label label-primary">Re-Stock</a>';
						$editBtn = '<a href="editbar2.php?id='.$rs["id"].'" class="label label-primary">Edit</a>';
