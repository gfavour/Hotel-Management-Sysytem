                      ...
                      <div class="box-header with-border">
                        <h4 class="box-title">
                          <a data-toggle="collapse" data-parent="#accordion" href="#collapseOne">
                            Reservation Booking (Free)<span style=" font:14px Arial, Helvetica, sans-serif; color:#c00; text-align:center; margin:0 0 10px 0;" id="rmleftgroup"></span> 
                          </a>
                        </h4>
                      </div>
  ...
  <option value="0">Choose Room</option>
    <?php
			  ...
			  while($addroom = mysqli_fetch_assoc($query)){
                echo '<option value="'.$addroom['id'].'">'.getOneField("SELECT catname FROM tblcategory WHERE id = ".mysqli_real_escape_string($conn4as,$addroom["categoryid"]),"catname").' ('.$addroom['roomType'].')</option>';
			  }
			  ?>
  </select>
  </div>
...
<div class="col-sm-4">
 ...
 <input type="date" id="checkin" class="form-control" name="checkin"  title="Check in date required" value="<?php echo date("Y-m-d"); ?>" onchange="getCheckInOutIntervalFree()">
  </div>
</div>
...
<div class="col-sm-4">
  ...
  <div class="form-group input-group">
  <span class="input-group-addon"><i class="fa fa-calendar"></i></span>
 <input type="date" id="checkout" class="form-control" name="checkout"  title="Check out date required" value="" onchange="getCheckInOutIntervalFree()">
  </div>
</div>
...
<div class="col-sm-4">
  ...
  <div class="form-group input-group">
 <input type="number" step="any" class="form-control" placeholder="" name="discount" id="discount" value="0" readonly>
  </div>
</div>
