			...
			<!--box-header -->
				...
				<div class="box-body table-responsive no-padding" style="margin-top:10px;">
                  <form name="reservation" id="reservation">
					<?php
					if ($_GET["dwat"] == 'search'){
					$sql = select2nav("SELECT count(id) as ctn FROM addbar WHERE (name LIKE '%".mysqli_real_escape_string($conn4as,$_GET["q"])."%') ORDER BY id DESC","SELECT * FROM addbar WHERE (name LIKE '%".mysqli_real_escape_string($conn4as,$_GET["q"])."%') ORDER BY id DESC");
					}else{
					$sql = select2nav("SELECT count(id) as ctn FROM addbar ORDER BY id","SELECT * FROM addbar ORDER BY id DESC");
					}
					if($sql){
