...
<div class="row">
            <div class="col-xs-12">
  <?php
  if (select("SELECT * FROM addbar WHERE id = ".mysqli_real_escape_string($conn4as,$_GET["id"]))){
  	$rs = mysqli_fetch_assoc($qry);
  }else{
  	redirect('manage-bar.php?m=record not found');
  }
...
<input type="hidden" name="dwat" value="restockbar" />
...
<input type="hidden" name="hidoldremstock" value="<?php echo $rs["oldremstock"]; ?>" />
...
echo "<strong style='width:200px;display:inline-block;'>Cost Price:</strong> ".$rs["costprice"]."<br>";
...
echo "<strong style='width:200px;display:inline-block;color:#f00;'>Instruction from Admin:</strong> re-stock ".GetInstructions('bar',$rs["id"]).' '.$rs["measure"]."(s)<br><br>";
?>
...
<a href="manage-bar.php" class="btn btn-sm btn-default" style="margin-right:10px;">Back to Manage Bar</a><button class="btn btn-warning btn-sm">Add Stock</button>
</div>
</form>
</div>
</div>
