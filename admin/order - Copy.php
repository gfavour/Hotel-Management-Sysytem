...
<section class="content">
  ...
  <select name="clientid" id="clientid" required x-moz-errormessage="Select a client." class="selectpicker" data-live-search="true">
        <?php
          //CustomersOptionByRoomOnly($_GET['clientid']);
 ...
 <div class="col-md-12" style="text-align:left; margin:10px 0 15px 0; padding-left:0;">
   <div style="font-size:18px; line-height:20px; margin:0 0 10px 0px; padding:5px 10px;border-radius:3px; text-align:left; background:#eee;">Select service(s) to place order.</div>
   
   <?php //if ($globalrole == 'admin' || $globalrole == 'manager' || $globalrole == 'receptionist'){ ?>
   <?php //if($_GET["clientid"] != 1){ ?>
   <!--<div class="col-sm-3" style="display:none;padding-left:0; text-align:left">
   ...
   <?php 
  // }else{
   //echo ($globalrole == 'receptionist')?'<div style="color:#fff;background:#e00;padding:10px;border-radius:5px;">You can\'t assign room to anonymous guest. Register the guest and assign room.</div>':'';
   //}}
   ?>
   
   <?php if ($globalrole == 'admin' || $globalrole == 'manager' || $globalrole == 'bar'){ ?>
   <!--<div class="col-sm-3" style="display:none;padding-left:0;">
   ...
   <?php } ?>
   <?php 
   if ($globalrole == 'admin' || $globalrole == 'manager' || $globalrole == 'spa'){ ?>
   <div class="col-sm-3" style="padding-left:0;">
