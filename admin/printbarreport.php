...
$sql = select("SELECT GROUP_CONCAT(addbar.name) AS aggname,SUM(order_bar.qty) AS groupqty,SUM(order_bar.total) AS grouptotal, order_bar.*,addbar.name FROM order_bar INNER JOIN addbar ON order_bar.itemid = addbar.id WHERE ".$filter_qry." GROUP BY order_bar.invoiceid ORDER BY order_bar.id DESC");
