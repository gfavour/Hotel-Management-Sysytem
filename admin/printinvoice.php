...
<div class="inner"><h3 style="font-size:3.5rem!important;"><?php $deposit = returnField("SELECT amount FROM addclient WHERE id = '".mysqli_real_escape_string($conn4as,$guestid)."' ORDER BY id","amount"); echo number_format($deposit,2);?></h3>
<p>Deposit</p>
</div></div></div>
<!--Grand Debt-->
<div class="col-lg-3 col-xs-6" style="cursor:pointer;">
				...
				<div class="box-body table-responsive no-padding" style="margin-top:10px;">
				  ...
				  echo '<div style="text-decoration:underlined;padding:10px; font-size:16px; text-align:left;"><strong><u>Room Booking</u></strong></div>';
					...
					while($rs = mysqli_fetch_assoc($qry)){
						...
						echo '<div><strong>Duration: </strong>'.$rs["duration"].' <strong>Discount: </strong>'.number_format($rs["discount"],4).'% <strong>VAT: </strong>'.$rs["vat"].'%</div>';
				...
				<div class="box-body table-responsive no-padding" style="margin-top:10px;">
				  ...
				  echo '<div style="text-decoration:underlined;padding:10px; font-size:16px; text-align:left;"><strong><u>Laundary Service</u></strong></div>';
					...
					while($rs = mysqli_fetch_assoc($qry)){
						...
						echo '<div><strong>Discount: </strong>'.number_format($rs["discount"],4).'%</div>';
				...
				<div class="box-body table-responsive no-padding" style="margin-top:10px;">
				  ...
				  echo '<div style="padding:10px; font-size:16px; text-align:left;"><strong><u>Spa Service</u></strong></div>';
					...
					while($rs = mysqli_fetch_assoc($qry)){
						...
						echo '<div><strong>No of person: </strong>'.$rs["noofperson"].' <strong>Discount: </strong>'.number_format($rs["discount"],4).'%</div>';
						echo '<div><strong>Price: </strong>'.$cursign.$rs["unitprice"].' <strong>Total: </strong>'.$cursign.number_format($rs["total"],2).' <strong>Date: </strong>'.$rs["orderdate"].'</div>';
						echo '</div>';
					}
					}
