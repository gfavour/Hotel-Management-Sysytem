...
<div class="row">
            <div class="col-xs-12">
			  ...
			  <div style="margin-bottom:10px">
                <button class="btn btn-primary btn-sm" style="margin-right:10px;" id="jqbtn1"><i class="fa fa-plus"></i> Add New Item</button> <a href="order-res-home.php" class="btn btn-warning btn-sm" style="margin-right:10px;"><i class="fa fa-plus"></i> New Order</a>
                </div>
			  <?php } ?>
			...
			<!--box-header -->
				...
				<div class="box-body table-responsive no-padding" style="margin-top:10px;">
                  <form name="reservation" id="reservation">
					<?php
					if ($_GET["dwat"] == 'search'){
					$sql = select2nav("SELECT count(id) as ctn FROM addresturant WHERE (name LIKE '%".mysqli_real_escape_string($conn4as,$_GET["q"])."%') ORDER BY id DESC","SELECT * FROM addresturant WHERE (name LIKE '%".mysqli_real_escape_string($conn4as,$_GET["q"])."%') ORDER BY id DESC");
					}else{
				  ...
				  echo '<table class="table table-striped">
					...
					if ($rs["ingredients"] != ''){ 
						$ingredients = 'N/A';
						$ingredients = GetIngredients(explode(",",$rs["ingredients"]));						
					}else{ 
						$ingredients = 'N/A'; 
					}
