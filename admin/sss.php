		...
		<div id="msgbox"></div>
			...
			<!--box-header -->
				...
				<div class="box-body table-responsive no-padding" style="margin-top:10px;">
				  ...
				  //$sql = select2nav("SELECT count(id) as ctn FROM order_room WHERE (orderdate BETWEEN '".mysqli_real_escape_string($conn4as,$_REQUEST["datefrom"])."' AND '".mysqli_real_escape_string($conn4as,$_REQUEST["dateto"])."') ORDER BY id","SELECT * FROM order_room WHERE (orderdate BETWEEN '".mysqli_real_escape_string($conn4as,$_REQUEST["datefrom"])."' AND '".mysqli_real_escape_string($conn4as,$_REQUEST["dateto"])."') ORDER BY id");
				  $sql = select2nav("SELECT count(order_room.id) as ctn FROM order_room INNER JOIN addclient ON order_room.guestid = addclient.id WHERE (order_room.orderdate BETWEEN '".mysqli_real_escape_string($conn4as,$_REQUEST["datefrom"])."' AND '".mysqli_real_escape_string($conn4as,$_REQUEST["dateto"])."') ORDER BY order_room.id DESC","SELECT order_room.*,addclient.lastname,addclient.firstname,addclient.phone FROM order_room INNER JOIN addclient ON order_room.guestid = addclient.id WHERE (order_room.orderdate BETWEEN '".mysqli_real_escape_string($conn4as,$_REQUEST["datefrom"])."' AND '".mysqli_real_escape_string($conn4as,$_REQUEST["dateto"])."') ORDER BY order_room.id DESC");	  
				  }else{
