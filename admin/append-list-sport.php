			...
			<!--box-header -->
				...
				<div class="box-body table-responsive no-padding" style="margin-top:10px;">
                  <form name="reservation" id="reservation">
					...
					$sql = select2nav("SELECT count(orders.id) as ctn FROM orders INNER JOIN addclient ON orders.guestid = addclient.id WHERE (orders.issport = '1' AND addclient.lastname LIKE '%".mysqli_real_escape_string($conn4as,$_POST["q"])."%') AND orders.ispaid <> '1' ORDER BY orders.id DESC","SELECT orders.*,addclient.lastname,addclient.firstname FROM orders INNER JOIN addclient ON orders.guestid = addclient.id WHERE (orders.issport = '1' AND addclient.lastname LIKE '%".mysqli_real_escape_string($conn4as,$_POST["q"])."%') AND orders.ispaid <> '1' ORDER BY orders.id DESC");
					
					}else{
					$sql = select2nav("SELECT count(orders.id) as ctn FROM orders INNER JOIN addclient ON orders.guestid = addclient.id WHERE orders.issport = '1' AND  orders.ispaid <> '1' ORDER BY orders.id DESC","SELECT orders.*,addclient.lastname,addclient.firstname FROM orders INNER JOIN addclient ON orders.guestid = addclient.id WHERE orders.issport = '1' AND orders.ispaid <> '1' ORDER BY orders.id DESC");
					}
					if($sql){
