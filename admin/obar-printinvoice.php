...
<body>
    ...
    <div>
		...
		/*
		$qry = mysqli_query($conn4as,"SELECT id,invoiceid,roomid,checkstatus FROM order_room WHERE invoiceid = '".mysqli_real_escape_string($conn4as,$_GET["invoiceid"])."' ORDER BY id DESC");
		$rs = mysqli_fetch_assoc($qry);
		...
		$sql = select("SELECT orders.invoiceid,orders.guestid,orders.ispaid,addclient.lastname,addclient.firstname FROM orders inner join addclient on orders.guestid = addclient.id WHERE orders.invoiceid = ".mysqli_real_escape_string($conn4as,$_GET["invoiceid"])." ORDER BY orders.id DESC");
		$rs = mysqli_fetch_assoc($qry);
