...
<div class="row">
            <div class="col-xs-12">
  <?php
  if (select("SELECT * FROM addbar3 WHERE id = ".mysqli_real_escape_string($conn4as,$_GET["id"]))){
  	$rs = mysqli_fetch_assoc($qry);
  }else{
  	redirect('manage-bar3.php?m=record not found');
  }
...
<input type="hidden" name="dwat" value="restockbar3" />
...
echo "<strong style='width:200px;display:inline-block;color:#f00;'>Instruction from Admin:</strong> re-stock ".GetInstructions('bar3',$rs["id"]).' '.$rs["measure"]."(s)<br><br>";
?>
...
<a href="manage-bar3.php" class="btn btn-sm btn-default" style="margin-right:10px;">Back to Manage Bar</a><button class="btn btn-warning btn-sm">Add Stock</button>
</div>
</form>
</div>
</div>
