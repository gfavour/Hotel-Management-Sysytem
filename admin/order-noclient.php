...
<input type="hidden" name="roomnoname" id="roomnoname" value="<?php echo $_GET['roomType']; ?>">
  <input type="hidden" name="paystatus" id="paystatus" value="<?php echo $_GET['pays']; ?>">
  <input type="hidden" name="invoiceid" value="<?php echo $invoiceid; ?>">
  <input type="hidden" name="grandtotal" value="0">
  <input type="hidden" name="ndtotal" id="ndtotal" value="0">
...
<?php
  $sql = "SELECT addroom.id,addroom.roomType,addroom.roomrate,addroom.rcardno,tblcategory.catname FROM addroom INNER JOIN tblcategory ON addroom.categoryid = tblcategory.id WHERE addroom.id=".mysqli_real_escape_string($conn4as,$_GET["roomid"])." AND tblcategory.cattype = 'room' ORDER BY addroom.id";
...
<div style="margin:8px 0 0 0; padding-top:5px; text-align:left;border-top:#ccc dotted 1px;"><label for="pay4rmewallet"><input type="checkbox" value="1" id="pay4rmewallet" name="pay4rmewallet"<?php echo ($_GET['pays'] == '1')?' checked':''; ?>> Deduct Payment From e-Wallet</label></div>
...
function DiscountCalculator(val,t,resID,totalid){
var dis = 0,toDiscount = 0,total = document.getElementById(totalid).value;
if(t == 1 && total > 0){
	dis = (parseFloat(val)*100)/parseFloat(total); //amount to percent
	dis = dis.toFixed(2);
	document.getElementById(resID).value = dis;
	toDiscount = dis;
}else if(t == 2 && total > 0){
	dis = parseFloat(total) * parseFloat(val)/100; //percent to amount
	dis = dis.toFixed(2);
	document.getElementById(resID).value = dis;
	toDiscount = val;
}
document.getElementById("discount").value = toDiscount;
calculate();
