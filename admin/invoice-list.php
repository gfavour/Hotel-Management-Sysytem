...
<div class="content-wrapper">
        <!-- Page Header -->
		<?php
		$sql = select("SELECT orders.invoiceid,orders.ispaid,addclient.lastname,addclient.firstname FROM orders inner join addclient on orders.guestid = addclient.id WHERE orders.invoiceid = ".mysqli_real_escape_string($conn4as,$_GET["invoiceid"])." ORDER BY orders.id DESC");
...
<div class="row">
		...
		<div id="msgbox"></div>
			  ...
			  <div class="box box-primary">
				  ...
				  <?php
					$sql = select("SELECT orders.*,addclient.lastname,addclient.firstname,addclient.phone FROM orders INNER JOIN addclient ON orders.guestid = addclient.id WHERE orders.invoiceid = '".mysqli_real_escape_string($conn4as,$_GET["invoiceid"])."' ORDER BY orders.id DESC");
					if($sql){				  
				...
				<div class="box-body table-responsive no-padding" style="margin-top:10px;">
				  <?php
					$sql = select2nav("SELECT count(id) as ctn FROM order_bar WHERE invoiceid = '".mysqli_real_escape_string($conn4as,$_GET["invoiceid"])."' ORDER BY id DESC","SELECT order_bar.*,addbar.name FROM order_bar inner join addbar on order_bar.itemid = addbar.id WHERE order_bar.invoiceid = '".mysqli_real_escape_string($conn4as,$_GET["invoiceid"])."' ORDER BY order_bar.id DESC");
					if($sql){
				  echo '<table class="table table-striped">
                    <div style=" background:#ffd; padding:10px; font-size:16px; text-align:center;"><strong>Bar-1 Service</strong></div><tr>
                      <th>SN</th>
                      <th>Invoice ID</th>
                      <th>Item</th>
					  <th>Quantity</th>
				...
				<div class="box-body table-responsive no-padding" style="margin-top:10px;">
				  <?php
					$sql = select2nav("SELECT count(id) as ctn FROM order_bar2 WHERE invoiceid = '".mysqli_real_escape_string($conn4as,$_GET["invoiceid"])."' ORDER BY id DESC","SELECT order_bar2.*,addbar2.name FROM order_bar2 inner join addbar2 on order_bar2.itemid = addbar2.id WHERE order_bar2.invoiceid = '".mysqli_real_escape_string($conn4as,$_GET["invoiceid"])."' ORDER BY order_bar2.id DESC");
					if($sql){
				  echo '<table class="table table-striped">
                    <div style=" background:#ffd; padding:10px; font-size:16px; text-align:center;"><strong>Bar-2 Service</strong></div><tr>
                      <th>SN</th>
                      <th>Invoice ID</th>
                      <th>Item</th>
					  <th>Quantity</th>
				...
				<div class="box-body table-responsive no-padding" style="margin-top:10px;">
				  <?php
					$sql = select2nav("SELECT count(id) as ctn FROM order_bar3 WHERE invoiceid = '".mysqli_real_escape_string($conn4as,$_GET["invoiceid"])."' ORDER BY id DESC","SELECT order_bar3.*,addbar3.name FROM order_bar3 inner join addbar3 on order_bar3.itemid = addbar3.id WHERE order_bar3.invoiceid = '".mysqli_real_escape_string($conn4as,$_GET["invoiceid"])."' ORDER BY order_bar3.id DESC");
					if($sql){
				  echo '<table class="table table-striped">
                    <div style=" background:#ffd; padding:10px; font-size:16px; text-align:center;"><strong>Bar-3 Service</strong></div><tr>
                      <th>SN</th>
                      <th>Invoice ID</th>
                      <th>Item</th>
					  <th>Quantity</th>
				...
				<div class="box-body table-responsive no-padding" style="margin-top:10px;">
				  <?php
					$sql = select2nav("SELECT count(id) as ctn FROM order_laundary WHERE invoiceid = '".mysqli_real_escape_string($conn4as,$_GET["invoiceid"])."' ORDER BY id DESC","SELECT order_laundary.*,addlaundary.title FROM order_laundary inner join addlaundary on order_laundary.itemid = addlaundary.id WHERE order_laundary.invoiceid = '".mysqli_real_escape_string($conn4as,$_GET["invoiceid"])."' ORDER BY order_laundary.id DESC");
					if($sql){
				  echo '<table class="table table-striped">
				...
				<div class="box-body table-responsive no-padding" style="margin-top:10px;">
				  <?php
					$sql = select2nav("SELECT count(id) as ctn FROM order_spa WHERE invoiceid = '".mysqli_real_escape_string($conn4as,$_GET["invoiceid"])."' ORDER BY id DESC","SELECT order_spa.*,addspa.name FROM order_spa inner join addspa on order_spa.itemid = addspa.id WHERE order_spa.invoiceid = '".mysqli_real_escape_string($conn4as,$_GET["invoiceid"])."' ORDER BY order_spa.id DESC");
					if($sql){
				  echo '<table class="table table-striped">
