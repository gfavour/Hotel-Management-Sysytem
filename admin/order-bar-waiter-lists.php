			...
			<!--box-header -->
				...
				<div class="box-body table-responsive no-padding" style="margin-top:10px;">
                  <form name="reservation" id="reservation">
					...
					//$sql = select2nav("SELECT count(order_bar.id) as ctn FROM order_bar INNER JOIN addbar ON order_bar.itemid = addbar.id WHERE (addbar.name LIKE '%".$_POST["q"]."%' OR order_bar.waiter LIKE '%".$_POST["q"]."%' OR order_bar.invoiceid LIKE '%".$_POST["q"]."%') AND order_bar.isflag = '0' GROUP BY order_bar.invoiceid  ORDER BY order_bar.id","SELECT GROUP_CONCAT(addbar.name) AS aggname,SUM(order_bar.qty) AS groupqty,SUM(order_bar.total) AS grouptotal,order_bar.*,addbar.name FROM order_bar INNER JOIN addbar ON order_bar.itemid = addbar.id WHERE (addbar.name LIKE '%".$_POST["q"]."%' OR order_bar.waiter LIKE '%".$_POST["q"]."%' OR order_bar.invoiceid LIKE '%".$_POST["q"]."%') AND order_bar.isflag = '0' GROUP BY order_bar.invoiceid  ORDER BY order_bar.id DESC");
					$sql = select2nav("SELECT count(order_bar.id) as ctn FROM order_bar INNER JOIN addbar ON order_bar.itemid = addbar.id WHERE (addbar.name LIKE '%".$_POST["q"]."%' OR order_bar.waiter LIKE '%".$_POST["q"]."%' OR order_bar.invoiceid LIKE '%".$_POST["q"]."%') AND order_bar.isflag = '0' ORDER BY order_bar.id","SELECT order_bar.*,addbar.name FROM order_bar INNER JOIN addbar ON order_bar.itemid = addbar.id WHERE (addbar.name LIKE '%".$_POST["q"]."%' OR order_bar.waiter LIKE '%".$_POST["q"]."%' OR order_bar.invoiceid LIKE '%".$_POST["q"]."%') AND order_bar.isflag = '0' ORDER BY order_bar.id DESC");
					}else{
					//$sql = select2nav("SELECT count(order_bar.id) as ctn FROM order_bar INNER JOIN addbar ON order_bar.itemid = addbar.id WHERE order_bar.isflag = '0' GROUP BY order_bar.invoiceid ORDER BY order_bar.id","SELECT GROUP_CONCAT(addbar.name) AS aggname,SUM(order_bar.qty) AS groupqty,SUM(order_bar.total) AS grouptotal,order_bar.*,addbar.name FROM order_bar INNER JOIN addbar ON order_bar.itemid = addbar.id WHERE order_bar.isflag = '0' GROUP BY order_bar.invoiceid ORDER BY order_bar.id DESC");
					$sql = select2nav("SELECT count(order_bar.id) as ctn FROM order_bar INNER JOIN addbar ON order_bar.itemid = addbar.id WHERE order_bar.isflag = '0' ORDER BY order_bar.id","SELECT order_bar.*,addbar.name FROM order_bar INNER JOIN addbar ON order_bar.itemid = addbar.id WHERE order_bar.isflag = '0' ORDER BY order_bar.id DESC");
					}
