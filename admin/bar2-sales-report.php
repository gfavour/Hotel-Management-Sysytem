...
<?php 
if(isset($_REQUEST["datefrom"])){
if($sqlwaiter != ''){ $sqlw = " LOWER(order_bar2.waiter) = '".mysqli_real_escape_string($conn4as,strtolower($sqlwaiter))."' AND "; }else{ $sqlw = ""; }
$filter_qry = $sqlw." TIMESTAMP(order_bar2.orderdate, order_bar2.ordertime) BETWEEN TIMESTAMP('".mysqli_real_escape_string($conn4as,$_REQUEST["datefrom"])."','".mysqli_real_escape_string($conn4as,$_REQUEST["timefrom"])."') AND TIMESTAMP('".mysqli_real_escape_string($conn4as,$_REQUEST["dateto"])."','".mysqli_real_escape_string($conn4as,$_REQUEST["timeto"])."') ";
...
$sql = select2nav("SELECT count(order_bar2.id) as ctn FROM order_bar2 INNER JOIN addbar2 ON order_bar2.itemid = addbar2.id WHERE ".$filter_qry." ORDER BY order_bar2.id DESC","SELECT order_bar2.*,addbar2.name FROM order_bar2 INNER JOIN addbar2 ON order_bar2.itemid = addbar2.id WHERE ".$filter_qry." ORDER BY order_bar2.id DESC");

$grandtotal = getGrandTotal("SELECT SUM(order_bar2.total) AS grandtotal FROM order_bar2 WHERE ".$filter_qry." ORDER BY id");
$paidtotal = getGrandTotal("SELECT SUM(order_bar2.total) AS grandtotal FROM order_bar2 WHERE order_bar2.ispaid = '1' AND ".$filter_qry." ORDER BY id");
$unpaidtotal = getGrandTotal("SELECT SUM(order_bar2.total) AS grandtotal FROM order_bar2 WHERE order_bar2.ispaid <> '1' AND ".$filter_qry." ORDER BY id");
