...
$sql = select("SELECT GROUP_CONCAT(addbar2.name) AS aggname,SUM(order_bar2.qty) AS groupqty,SUM(order_bar2.total) AS grouptotal, order_bar2.*,addbar2.name FROM order_bar2 INNER JOIN addbar2 ON order_bar2.itemid = addbar2.id WHERE ".$filter_qry." GROUP BY order_bar2.invoiceid ORDER BY order_bar2.id DESC");
