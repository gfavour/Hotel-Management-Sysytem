...
<div class="content-wrapper">
        ...
        <section class="content-header">
          <h1>Sport Materials Trend Analysis <?php if($_GET["month"] != ''){ echo '('.getMonth($_GET["month"]).', '.$_GET["year"].')'; } ?></h1>
          <ol class="breadcrumb">
            <li><a href="#"><i class="fa fa-dashboard"></i> Home</a></li>
            <li class="active">Sport Materials Trend Analysis</li>
          </ol>
        </section>
			...
			<!--box-header -->
				...
				<script>
                  ...
                  <form name="reservation" id="reservation">
					...
					$sql = select2nav("SELECT count(order_sportitem.id) as ctn FROM order_sportitem INNER JOIN addsport ON order_sportitem.itemid = addsport.id WHERE (order_sportitem.orderdate LIKE '".$odate."-%') GROUP BY order_sportitem.itemid ORDER BY order_sportitem.id ASC ","SELECT addsport.name,order_sportitem.itemid,SUM(order_sportitem.qty) AS qty,SUM(order_sportitem.total) AS total,order_sportitem.orderdate FROM order_sportitem INNER JOIN addsport ON order_sportitem.itemid = addsport.id WHERE (order_sportitem.orderdate LIKE '".$odate."-%') GROUP BY order_sportitem.itemid ORDER BY order_sportitem.id ASC");
					
					}else{
					$sql = select2nav("SELECT count(order_sportitem.id) as ctn FROM order_sportitem INNER JOIN addsport ON order_sportitem.itemid = addsport.id GROUP BY order_sportitem.itemid ORDER BY order_sportitem.id ASC ","SELECT addsport.name,order_sportitem.itemid,SUM(order_sportitem.qty) AS qty,SUM(order_sportitem.total) AS total,order_sportitem.orderdate FROM order_sportitem INNER JOIN addsport ON order_sportitem.itemid = addsport.id GROUP BY order_sportitem.itemid ORDER BY order_sportitem.id ASC");
					}
					if($sql){
