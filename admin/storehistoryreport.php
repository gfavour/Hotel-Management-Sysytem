...
<div class="content-wrapper">
        ...
        <section class="content-header">
          ...
          <ol class="breadcrumb">
            ...
            <li class="active">Store History</li>
          </ol>
        </section>
...
<div class="row">
		...
		<div id="msgbox"></div>
              ...
              <h3 class="box-title"><?php if($_REQUEST["datefrom"] != '' && $_REQUEST["dateto"] != ''){ echo 'Store History Between '.$_REQUEST["datefrom"].' - '.$_REQUEST["dateto"]; }elseif($_REQUEST["datefrom"] != '' && $_REQUEST["dateto"] == ''){ echo 'Store History From '.$_REQUEST["datefrom"]; }elseif($_REQUEST["datefrom"] == '' && $_REQUEST["dateto"] != ''){ echo 'Store History Till '.$_REQUEST["dateto"]; }else{ echo ' Store History Till Date'; } ?></h3>
              <div class="box-tools pull-right">
			  <a href="printstorehistoryreport.php?<?php echo 'itemname='.$_REQUEST["itemname"].'&datefrom='.$_REQUEST["datefrom"].'&dateto='.$_REQUEST["dateto"].'&arrangeby='.$_REQUEST["arrangeby"]; ?>" class="label label-info" target="_blank">Print</a>
			...
			<!--box-header -->
				...
				<div class="box-body table-responsive no-padding" style="margin-top:10px;">
				  ...
				  if ($sqlqry != ''){
				  $sql = select2nav("SELECT count(tblstorehistory.id) as ctn FROM tblstorehistory INNER JOIN tblstore ON tblstorehistory.itemid = tblstore.id ".$sqlqry." ORDER BY tblstorehistory.id ".$arrangeby,"SELECT tblstorehistory.*,tblstore.name,tblstore.qtyinstore,tblstore.measure FROM tblstorehistory INNER JOIN tblstore ON tblstorehistory.itemid = tblstore.id ".$sqlqry." ORDER BY tblstorehistory.id ".$arrangeby);
				  }else{
				  $sql = select2nav("SELECT count(tblstorehistory.id) as ctn FROM tblstorehistory INNER JOIN tblstore ON tblstorehistory.itemid = tblstore.id ORDER BY tblstorehistory.id ".$arrangeby,"SELECT tblstorehistory.*,tblstore.name,tblstore.qtyinstore,tblstore.measure FROM tblstorehistory INNER JOIN tblstore ON tblstorehistory.itemid = tblstore.id ORDER BY tblstorehistory.id ".$arrangeby);
				  }
