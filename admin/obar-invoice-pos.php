				...
				<div style="margin-top:1px;">
				  <?php
					...
					while($rs = mysqli_fetch_assoc($qry)){
						...
						if ($rs["guestid"] <> '1'){
							echo '<div class="width-div-3">';
							echo ($showGCatList == 1)?'<strong>Item: '.strtoupper($rs["name"]).'</strong><br>':'<strong>Room Service Type: '.strtoupper($rs["roomitemcat"]).'</strong><br>';
							...
							//<br><strong>VAT: </strong>'.$rs["vat"].'%, <strong>Discount: </strong>'.number_format($rs["discount"],2).'%
							echo '<br><strong>Payment Status: </strong>'.$ispaid;
							echo '<br><strong>Date / Time: </strong>'.$rs["orderdate"].' '.hr24to12($rs["ordertime"]).', ';
				...
				<div style="margin-top:1px;">
				  <?php
					...
					while($rs = mysqli_fetch_assoc($qry)){
						...
						echo '<div><strong>Price: </strong>'.$cursign.$rs["unitprice"].', <strong>Sub Total: </strong>'.$cursign.number_format($rs["total"],2).' <strong>Date: </strong>'.$rs["orderdate"].'</div>';echo '<strong>Payment Status: </strong>'.$ispaid;
						echo '<br><strong>Date / Time: </strong>'.$rs["orderdate"].' '.hr24to12($rs["ordertime"]).', ';
						echo '</div>';
						$grandtotal_bar += $rs["total"];
						$totaldiscount += $rs["discount"];
