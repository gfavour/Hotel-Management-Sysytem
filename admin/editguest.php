...
<div class="row">
            <div class="col-xs-12">
  <?php
  if (select("SELECT * FROM addclient WHERE id = ".mysqli_real_escape_string($conn4as,$_GET["id"]))){
...
<input type="hidden" name="hidpic" value="<?php echo $rs["pic"]; ?>" />
...
<select name="company" id="company" title="company" class="form-control"><?php echo getAllCompanyOpt($rs["company"]); ?></select>
</div>  
</div>
...
<span class="input-group-addon"><i class="fa fa-building-o"></i></span>
  ...
  if(select("select * from tblcountry order by id")){
	  while($rs3 = mysqli_fetch_assoc($qry)){
			echo ($countryid == $rs3["country"])?'<option value="'.$rs3["country"].'" selected>'.$rs3["country"].'</option>':'<option value="'.$rs3["country"].'">'.$rs3["country"].'</option>';  
	  }
  }else{
...
<a href="manage-guests.php" class="btn btn-md btn-default" style="margin-right:10px;">Back to Manage Guests</a> <button class="btn btn-warning btn-md">Update</button> 
</div>
</form>
</div>
</div>
