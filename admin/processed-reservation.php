			...
			<!--box-header -->
				...
				<div class="box-body table-responsive no-padding" style="margin-top:10px;">
                  <form name="reservation" id="reservation">
					<?php
					$sql = select2nav("SELECT count(id) as ctn FROM onlineorders WHERE status = '2' ORDER BY id DESC","SELECT * FROM onlineorders WHERE status = '2' ORDER BY id DESC");
					if($sql){
				  echo '<table class="table table-striped">
					...
					echo '<div align="center">No order processed.</div>';
					}
				?>
				</form>
                </div><!-- /.box-body -->
