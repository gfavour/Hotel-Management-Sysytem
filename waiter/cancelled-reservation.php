			...
			<!--box-header -->
				...
				<div class="box-body table-responsive no-padding" style="margin-top:10px;">
                  <form name="reservation" id="reservation">
					<?php
					$sql = select2nav("SELECT count(id) as ctn FROM onlineorders WHERE status = '3' ORDER BY id DESC","SELECT * FROM onlineorders WHERE status = '3' ORDER BY id DESC");
					if($sql){
				  echo '<table class="table table-striped">
					...
					$delBtn = '<a href="javascript:;" class="label label-danger" onclick="confirmdel(\'id='.$rs["id"].'&tbl=onlineorders&dwat=del\',\'../fxn/process1.php\',\'\')">Delete</a>';
					
					$viewBtn = '<a href="javascript:;" class="label label-success" onclick="showmodal(\''.$customerdetails.'\',\'<strong style=color:#fff;>Order Detais</strong>\',\'#3C8DBC\')" title="Cancel online order">View order</a>';
					...
					echo '<div align="center">No order cancelled.</div>';
					}
				?>
				</form>
                </div><!-- /.box-body -->
