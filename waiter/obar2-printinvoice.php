...
<body>
    ...
    <div class="row" style="padding:0px;">
		...
		$qry = mysqli_query($conn4as,"SELECT id,invoiceid,guestid,ispaid FROM order_bar2 WHERE invoiceid = '".mysqli_escape_string($conn4as,$_GET["invoiceid"])."' ORDER BY id DESC");
		...
		$qry = mysqli_query($conn4as,"SELECT id,invoiceid,guestid,ispaid FROM order_swimpool WHERE invoiceid = '".mysqli_escape_string($conn4as,$_GET["invoiceid"])."' ORDER BY id DESC");
		$totalinv = mysqli_num_rows($qry);
		if($totalinv > 0){
			$rs = mysqli_fetch_assoc($qry);
			if($rs["ispaid"] <> '1'){
