...
<div id="msgbox"></div>
<?php include("order-swimpool-inc.php"); ?>
<div>
			...
			<!--box-header -->
				...
				<div class="box-body table-responsive no-padding" style="margin-top:10px;">
                  <form name="reservation" id="reservation">
					...
					$sql = select2nav("SELECT count(order_swimpool.id) as ctn FROM order_swimpool INNER JOIN addclient ON order_swimpool.guestid = addclient.id WHERE (addclient.lastname LIKE '%".$_POST["q"]."%' OR addclient.firstname LIKE '%".$_POST["q"]."%' OR order_swimpool.invoiceid LIKE '%".$_POST["q"]."%') GROUP BY order_swimpool.invoiceid ORDER BY order_swimpool.id DESC","SELECT order_swimpool.*,SUM(order_swimpool.total) AS gtotal, addclient.lastname,addclient.firstname FROM order_swimpool INNER JOIN addclient ON order_swimpool.guestid = addclient.id WHERE (addclient.lastname LIKE '%".$_POST["q"]."%' OR addclient.firstname LIKE '%".$_POST["q"]."%' OR order_swimpool.invoiceid LIKE '%".$_POST["q"]."%') GROUP BY order_swimpool.invoiceid ORDER BY order_swimpool.id DESC");
					
					}else{
					$sql = select2nav("SELECT count(order_swimpool.id) as ctn FROM order_swimpool INNER JOIN addclient ON order_swimpool.guestid = addclient.id GROUP BY order_swimpool.invoiceid ORDER BY order_swimpool.id DESC","SELECT order_swimpool.*,SUM(order_swimpool.total) AS gtotal, addclient.lastname,addclient.firstname FROM order_swimpool INNER JOIN addclient ON order_swimpool.guestid = addclient.id GROUP BY order_swimpool.invoiceid ORDER BY order_swimpool.id DESC");
					}
