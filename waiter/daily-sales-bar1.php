...
<?php 
if(isset($_REQUEST["datefrom"])){
$filter_qry = " LOWER(order_bar.waiter) = '".mysqli_escape_string(strtolower($globalwname))."' AND TIMESTAMP(order_bar.orderdate, order_bar.ordertime) BETWEEN TIMESTAMP('".mysqli_escape_string($conn4as,$_REQUEST["datefrom"])."','".mysqli_escape_string($conn4as,$_REQUEST["timefrom"])."') AND TIMESTAMP('".mysqli_escape_string($conn4as,$_REQUEST["dateto"])."','".mysqli_escape_string($conn4as,$_REQUEST["timeto"])."') ";
