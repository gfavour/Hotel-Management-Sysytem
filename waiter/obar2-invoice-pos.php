...
<div style="font-size:10px !important;">
				...
				<div style="margin-top:1px;">
				  <?php
					...
					while($rs = mysqli_fetch_assoc($qry)){
						...
						echo '<br><strong>Price: </strong>'.$cursign.$rs["unitprice"].$sclabel.'<br><strong>Discount: </strong>'.$rs["discount"].'%<br><strong>Total: </strong>'.$cursign.number_format($rs["total"],2);
				...
				<div style="margin-top:1px;">
				  <?php
					...
					while($rs = mysqli_fetch_assoc($qry)){
						...
						echo '<div><strong>Price: </strong>'.$cursign.$rs["unitprice"].', <strong>Discount: </strong>'.$rs["discount"].'%, <strong>Total: </strong>'.$cursign.number_format($rs["total"],2).' <strong>Date/Time: </strong>'.$rs["orderdate"].' '.$rs["ordertime"].'</div>';echo '<strong>Payment Status: </strong>'.$ispaid;
						echo '<br><strong>Date / Time: </strong>'.$rs["orderdate"].' '.hr24to12($rs["ordertime"]).', ';
						echo '</div>';
						$grandtotal_bar += $rs["total"];
					}
				...
				<?php 
				//}
				echo '<div><strong>GRAND TOTAL: </strong>'.$cursign.number_format($grandtotal_bar,2).'</div>'; 
				?>
