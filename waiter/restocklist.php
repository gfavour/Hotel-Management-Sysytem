		...
		<div id="msgbox"></div>
			...
			<!--box-header -->
				...
				<div class="box-body table-responsive no-padding" style="margin-top:10px;">
				  ...
				  $sql = select2nav("SELECT count(tblrestock.id) as ctn FROM tblrestock INNER JOIN addbar ON tblrestock.itemid = addbar.id WHERE (tblrestock.regdate BETWEEN '".mysqli_escape_string($conn4as,$_REQUEST["datefrom"])."' AND '".mysqli_escape_string($conn4as,$_REQUEST["dateto"])."') ORDER BY tblrestock.id DESC","SELECT tblrestock.*,addbar.name,addbar.measure FROM tblrestock INNER JOIN addbar ON tblrestock.itemid = addbar.id WHERE (tblrestock.regdate BETWEEN '".mysqli_escape_string($conn4as,$_REQUEST["datefrom"])."' AND '".mysqli_escape_string($conn4as,$_REQUEST["dateto"])."') ORDER BY tblrestock.id DESC");	  
				  }else{
