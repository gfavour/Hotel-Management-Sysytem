				...
				<div style="margin-top:1px;">
				  <?php
					...
					while($rs = mysqli_fetch_assoc($qry)){
						...
						echo '<div><strong>Price: </strong>'.$cursign.$rs["unitprice"].', <strong>Discount: </strong>'.$rs["discount"].'%, <strong>Total: </strong>'.$cursign.number_format($rs["total"],2).' <strong>Date: </strong>'.$rs["orderdate"].'</div>';echo '<strong>Payment Status: </strong>'.$ispaid;
						echo '<br><strong>Date / Time: </strong>'.$rs["orderdate"].' '.hr24to12($rs["ordertime"]).', ';
						echo '</div>';
						$grandtotal_bar += $rs["total"];
					}
