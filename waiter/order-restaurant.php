...
<section class="content">
  ...
  <input type="hidden" name="dwat" value="ordermres">
  </div>
                </div>
...
<div class="col-sm-3">
  <div class="form-group input-group"><span class="input-group-addon">Discount</span>
   <input type="text" class="form-control" placeholder="" value="0" name="resdiscount" onchange="calculatemres()" onkeyup="calculatemres()">
...
<div class="col-sm-3">
 ...
 <input type="date" class="form-control" placeholder="" name="resorderdate" value="<?php echo date("m/d/Y"); ?>">
  </div>
</div>
...
<?php
  ...
  echo '<div style="margin:0 0 10px 0;"><strong>Select:</strong> <input type="checkbox" name="rescheck[]" value="'.$rs["id"].'" class="pull-right" onclick="calculatemres()"></div>';
  //echo '<div style="margin:0 0 10px 0;"><strong>Item Left:</strong> <span class="pull-right">'.$rs["itemleft"].' items</span></div>';
  echo '<div style="margin:0 0 10px 0;"><strong>Unit Price:</strong> <input type="text" readonly name="resprice'.$rs["id"].'" id="resprice'.$rs["id"].'" value="'.$rs["price"].'" class="pull-right" style="width:50px;padding-left:5px;"></div>';
  echo '<div style="margin:0 0 10px 0;"><strong>Quantity:</strong> <input type="text" name="resquantity'.$rs["id"].'" id="resquantity'.$rs["id"].'" value="1" class="pull-right" onchange="calculatemres()" onkeyup="calculatemres()" style="width:50px;padding-left:5px;"></div>';
  echo '<div style="margin:0 0 0 0;"><strong>Total:</strong> <input type="text" name="restotal'.$rs["id"].'" id="restotal'.$rs["id"].'" readonly value="" class="pull-right" style="width:50px;padding-left:5px;"></div>';
  echo '</div>';
  }
  }
  ?>						 
