  ...
  <option value="">Choose an Item</option>
    <?php
			  ...
			  while($addroom = mysqli_fetch_assoc($query)){
                echo '<option value="'.$addroom['id'].'">'.$addroom['name'].' ('.getOneField("SELECT catname FROM tblcategory WHERE id = ".mysqli_escape_string($conn4as,$addroom["categoryid"]),"catname").')</option>';
			  }
			  ?>
  </select>
  </div>
