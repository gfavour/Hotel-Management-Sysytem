...
<a href="javascript:window.print();" style="position:absolute;right:10px;top:10px;">Print Page</a>		               		
				  ...
				  $sql = select2nav("SELECT count(tblrestock.id) as ctn FROM tblrestock INNER JOIN addbar ON tblrestock.itemid = addbar.id WHERE (tblrestock.regdate BETWEEN '".mysqli_escape_string($conn4as,$_REQUEST["datefrom"])."' AND '".mysqli_escape_string($conn4as,$_REQUEST["dateto"])."') ORDER BY tblrestock.id","SELECT tblrestock.*,addbar.name,addbar.measure FROM tblrestock INNER JOIN addbar ON tblrestock.itemid = addbar.id WHERE (tblrestock.regdate BETWEEN '".mysqli_escape_string($conn4as,$_REQUEST["datefrom"])."' AND '".mysqli_escape_string($conn4as,$_REQUEST["dateto"])."') ORDER BY tblrestock.id");	  
				  }else{
