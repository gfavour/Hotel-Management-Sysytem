...
<?php
$oldroomid = $rs["roomid"];
$qr = mysqli_query($conn4as,"SELECT title,lastname,firstname FROM addclient WHERE id = ".mysqli_escape_string($conn4as,$rs["guestid"]));
  ...
  <option value="">Select...</option>
    <?php
			  ...
			  while($addroom = mysqli_fetch_assoc($query)){
                echo ($oldroomid == $addroom["id"] ||$addroom["currentinv"] != '')?'<option value="'.$addroom['id'].'" disabled style="background:#eee;"><strong>'.getOneField("SELECT catname FROM tblcategory WHERE id = ".mysqli_escape_string($conn4as,$addroom["categoryid"]),"catname").' ('.$addroom['roomType'].')&nbsp;&nbsp;&nbsp;-&nbsp;&nbsp;&nbsp;'.$cursign.number_format($addroom["roomrate"]).'</strong></option>':'<option value="'.$addroom['id'].'">'.getOneField("SELECT catname FROM tblcategory WHERE id = ".mysqli_escape_string($conn4as,$addroom["categoryid"]),"catname").' ('.$addroom['roomType'].')&nbsp;&nbsp;&nbsp;-&nbsp;&nbsp;&nbsp;'.$cursign.number_format($addroom["roomrate"]).'</option>';
			  }
			  ?>
  </select>
</div>
